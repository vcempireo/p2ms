...
- [ ] **【緊急】解析がまだ終わらない → API側の確認をお願い**

  症状: バナーは表示されるが解析が完了しない（永遠にくるくる）
  確認してほしいこと:
  1. `/api/food/analyze` のレスポンスタイムをログで確認
  2. Vercelのfunction logsでタイムアウトしていないか確認
  3. 2段階分割後のモデルは何を使っているか（gemini-1.5-flash になっているか）
  4. `maxDuration = 60` は設定されているか

- [x] **【緊急】解析が遅すぎる・コスト問題 → 2段階分割に変更してほしい**
